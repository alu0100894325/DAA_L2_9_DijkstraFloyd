...
import graph.Vertex;
/**
 * La clase FloydRecAlgorithm representa el algoritmo Floyd recursivo
 ...
 *
 */
public class FloydRecAlgorithm {
	 ...
	 * @param graph grafo
	 */
	public FloydRecAlgorithm (Graph graph) {
		setGraph(graph);
	...
	/**
	 * Ejecuta el algoritmo de Floyd recursivo.
	 */
	public void execute () {
		initialize();
		...
		for (int i = 0; i < getSide(); i++)
			for (int j = 0; j < getSide(); j++) {
				int result = recFunction(i, j, getSide() - 1);
				setElementWeights(i, j, result);
			}
	}
	
	/**
	 * Función que calcula los pesos mínimos de manera recursiva.
	 * @param i fila
	 * @param j columna
	 * @param k nodo a comprobar
	 * @return peso mínimo
	 */
	public int recFunction (int i, int j, int k) {
		
		if (k == 0)
			return getElementWeights(i, j);
		else {
			int first = recFunction(i, j, k - 1);
			int second = recFunction(i, k, k - 1) + recFunction(k, j, k - 1);
			if (first > second) {
